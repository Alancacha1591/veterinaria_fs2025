...
@section('content')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0">Lista de Veterinarios</h1>
        <div>
            <a href="{{ route('veterinarios.create') }}" class="btn btn-warning me-2">Nuevo</a>
        </div>
    </div>
            ...
            <div class="table-responsive">
                <table class="table table-striped table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        {{--Filas de veterinario --}}
                            <tr>
                                <th scope="row" class="text-center">{{ $veterinario->id }}</th>
                                <td class="text-center">{{ $veterinario->nombre }}</td>
                                <td class="text-center">{{ $veterinario->apellidos }}</td>
                                <td class="text-center">{{ $veterinario->edad }}</td>
                                <td class="text-center">{{ $veterinario->especialidad }}</td>
                                <td class="text-center">{{ $veterinario->telefono }}</td>
                                <td class="text-center">
                                    {{-- Botones de ver y editar --}}
                                    <a href="{{ route('veterinarios.show', $veterinario->id) }}" class="btn btn-sm btn-outline-primary">Ver</a>
                                    <a href="{{ route('veterinarios.edit', $veterinario->id) }}" class="btn btn-sm btn-outline-success">Editar</a>
